...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  const heightTransform = useTransform(
    scrollYProgress,
    [0, 1],
    [0, Math.min(height, containerHeight)]
  );
    ...
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        ...
        <motion.div
          ...
          className="absolute md:left-8 left-8 top-0 w-[2px] bg-gradient-to-t from-purple-500 via-neutral-800 to-transparent from-[0%] via-[10%] rounded-full overflow-hidden"
        >
          <div className="h-full w-full bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-[0%] via-neutral-200 dark:via-neutral-700 to-transparent to-[99%] [mask-image:linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]" />
        </motion.div>
      </div>
    </div>
  );
